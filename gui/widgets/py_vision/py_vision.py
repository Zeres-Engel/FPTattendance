        ...
        if self.status is True:
            ...
            for face in faces:
                ...
                if face.det_score > 0.68:
                    ...
                    # Kiểm tra tin cậy 
                    check = float(df.loc[df['Student ID'] == index_name, 'Confidence'].iloc[0])

                    #! Đánh dấu present cho gương mặt >= 0.5 và sẽ lấy gương mặt có tương quan cao nhất có thể
                    if max_similarity >= 0.5 and max_similarity > check:
                        #* Kiểm tra và cập nhật cột "Check-in"
                        if df.loc[(df['Student ID'] == index_name) & df['Check-in'].isna()].shape[0] > 0:
                            path = recognized_face_name + '/in.png'
                            path = path.replace('\\','/')
                            df.loc[(df['Student ID'] == index_name) & df['Check-in'].isna(), 'Check-in'] = str(path)
                        ...
                        path = path.replace('\\','/')
                        df.loc[(df['Student ID'] == index_name) & df['Check-out'].isna(), 'Check-out'] = str(path)
                        face_image = face_align.norm_crop(frame, face.kps)  # Đảm bảo bạn có face_align và frame ở đây
                        cv2.imwrite(path, face_image)
                        if df.loc[(df['Student ID'] == index_name) & ~df['Check-in'].isna() & ~df['Check-out'].isna()].shape[0] > 0:
                            df.loc[(df['Student ID'] == index_name), 'Attendance'] = 'Present'
                            
                        df.loc[(df['Student ID'] == index_name), 'Confidence'] = round(max_similarity, 2)
                ...
                #! Thực hiện nhận diện cho từng khuôn mặt có det_score cao hơn 0.68
                    ...
                    cv2.putText(frame_rgb, "Unknown", (bbox[0], bbox[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (242, 111, 33), 2)
